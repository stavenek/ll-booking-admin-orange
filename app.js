...
function fetchBookings() {
  ...
  fetch(`${API_BASE}/bookings?admin=${encodeURIComponent(adminPwd)}`)
    .then((res) => {
      if (!res.ok) {
...
function updateBookingStatus(bookingId, newStatus, selectEl) {
  ...
  // Update the status locally before sending
  booking.status = newStatus;

  // Save the full booking back to the correct API endpoint (no /status endpoint)
  const url = `${API_BASE}/bookings/${bookingId}?admin=${encodeURIComponent(adminPwd)}`;
  console.log("Saving booking to:", url);
  fetch(url, {
    method: 'PUT',
    headers: {
    ...
    .then(res => {
      ...
      console.log(`Booking ${bookingId} status changed to ${newStatus} and saved.`);
      renderTable();
...
function updateBookingTime(bookingId, newTime, selectEl) {
  console.log(`Changing time of booking ${bookingId} to ${newTime}`);
  selectEl.disabled = true;
  const booking = allBookings.find(b => b.bookingId === bookingId);
  if (!booking) {
    alert('Booking not found');
  ...
  fetch(url, {
    ...
    .then(res => {
      console.log(`Response status: ${res.status} ${res.statusText}`);
      if (!res.ok) throw new Error('Failed to save booking');
      console.log(`Booking ${bookingId} time changed to ${newTime} and saved.`);
      renderTable();
      if (typeof renderPagination === 'function') renderPagination();
    })
    .catch(err => {
